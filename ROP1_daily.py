...
# Параметры
TABLE_NAME = "ROP1"  # имя таблицы в ClickHouse

# Создаём DAG
dag = DAG(
    dag_id='mariadb_to_clickhouse_ROP1_daily',
    schedule_interval='0 8 * * *',
    start_date=pendulum.datetime(2025, 9, 9, tz="Europe/Moscow"),
    catchup=False,
...
def extract_from_file(**context):
    ...
    query = SQL_QUERY.replace("{{ yesterday_ds }}", f"'{yesterday}'")
...
def load_to_file(**context):
    try:
        ...
        # Удаление за вчера 
        delete_sql = f"ALTER TABLE {TABLE_NAME} DELETE WHERE date = '{yesterday}'"
        client.execute(delete_sql)

        # Ждем завершения удаления (синхронизируем)
        ...
        print(f"Error in load_to_file: {str(e)}")
        raise
